...
#+BEGIN_SRC emacs-lisp 
  (progn
    ...
    ; Find in project
    (define-key tp/ag-key-map (kbd "p") 'helm-multi-swoop-projectile)
    ; Find in file
    (define-key tp/ag-key-map (kbd "f") 'ag)
    ; Find using plain-old swoop :-)
    (define-key tp/ag-key-map (kbd "s") 'helm-swoop)
    ; Find using multi-swoop
    (define-key tp/ag-key-map (kbd "m") 'helm-multi-swoop)
    )
