...
*** Font Choices
 #+BEGIN_SRC emacs-lisp
   (set-face-attribute 'default nil :family "InputMonoCondensed" :height 120)
   (set-face-attribute 'fixed-pitch nil :family "InputMonoCondensed" :height 120)
   (cond
    ((string-equal system-type "windows-nt")
     (progn
       (set-face-attribute 'variable-pitch nil :family "InputSansCondensed" :height 120)))
    ((string-equal system-type "gnu/linux")
     (progn
       (set-face-attribute 'variable-pitch nil :family "InputSansCondensedLight" :height 120))))
 #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;       "%TODO %4PRIORITY(Pri.) %50ITEM(Task) %7POM_Pomodori(Poms) %12CLOCKSUM_T(Today's Time)")
...
#+END_SRC
    ...
    I've been using =pipenv= a lot lately on both Windows and Linux and it has
    dramatically simplified my workflow. So let's try this out:
