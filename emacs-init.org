...
#+END_SRC
    ...
    Since Olivetti breaks up long lines =C-k= (which maps to =kill-line=) doesn't actually kill
    the entire line. Thanks to Xah yet again I have a solution:
...
#+END_SRC
 ...
 #+BEGIN_SRC emacs-lisp
   ...
   (set-face-attribute 'variable-pitch nil :family "Sarabun Light")
 #+END_SRC

 #+RESULTS:

*** Viewing monospaced and non-monospaced fonts in the same document
    Emacs has a great feature that allows you to view non-code text
    using a proportional font (like Helvetica) and code text using a
    non-proportional font (like Courier). You just have to run this below:
